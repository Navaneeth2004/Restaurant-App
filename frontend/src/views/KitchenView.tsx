...
export default function KitchenView() {
  ...
  const load = useCallback(async () => {
    try { setOrders(await getActiveOrders()); } catch { toast('Failed to load orders', 'error'); }
  }, []);
  ...
  useSocket('order_updated', ({ order }: { order: Order }) => {
    setOrders(prev => prev.some(o => o.id === order.id)
      ? prev.map(o => o.id === order.id ? order : o) : prev);
  });
  ...
  // FIX #10: Only mark urgent after 20 minutes (more reasonable threshold)
  const urgentCount = orders.filter(o => isUrgent(o.created_at, 20)).length;

  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-shrink-0 flex items-center gap-3 px-5 py-3 border-b border-surface-border bg-surface-card/50">
        <h2 className="font-bold text-white text-sm">Kitchen Display</h2>
        {orders.length > 0 && (
          <span className="text-[10px] font-bold uppercase tracking-wide text-brand-400 bg-brand-500/15 border border-brand-500/25 px-2 py-0.5 rounded-full">
            {orders.length} active
          </span>
        )}
        {urgentCount > 0 && (
          <span className="text-[10px] font-bold uppercase tracking-wide text-red-400 bg-red-500/15 border border-red-500/25 px-2 py-0.5 rounded-full animate-pulse">
            {urgentCount} overdue
          </span>
        )}
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {orders.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {orders.map(order => {
              // FIX #10: Use 20 min threshold for urgent
                ...
                <div key={order.id} className={`rounded-xl border overflow-hidden flex flex-col animate-slide-up ${urgent ? 'border-red-500/60 shadow-lg shadow-red-500/10' : 'border-surface-border'}`}>
                  ...
                  <div className={`px-4 py-3 flex items-center justify-between ${urgent ? 'bg-red-500' : 'bg-brand-500'}`}>
                    <div className="flex items-center gap-2.5">
                      ...
                      {urgent && <span className="text-[9px] font-bold uppercase tracking-widest text-red-100">Overdue</span>}
                    </div>
                    <div className={`flex items-center gap-1 text-xs font-mono font-semibold px-2.5 py-1 rounded-full ${urgent ? 'bg-red-600/60 text-red-100' : 'bg-brand-600/60 text-brand-100'}`}>
                  ...
                  <div className="px-4 py-3 flex-1 space-y-2.5 bg-surface-card">
                    {order.items.map((item, i) => (
                      <div key={i} className="flex items-start gap-2.5">
                        <span className="font-mono font-bold text-brand-400 text-lg leading-none w-7 flex-shrink-0">{item.quantity}×</span>
