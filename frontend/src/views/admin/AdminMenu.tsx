    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="rounded-xl border border-surface-border bg-surface-card p-5 w-full max-w-lg animate-slide-up" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-3">
            <div>
              ...
              <input className="input" placeholder="Crispy Wings" value={name} onChange={e => setName(e.target.value)} autoFocus />
            </div>
            <div>
              <label className="label">Description</label>
              <textarea className="input resize-none" rows={2} placeholder="Short description…" value={desc} onChange={e => setDesc(e.target.value)} />
            </div>
          ...
          <div>
            ...
            <div
              className={`relative border-2 border-dashed rounded-xl overflow-hidden cursor-pointer transition-colors hover:border-brand-500/60 ${
                preview ? 'border-surface-border' : 'border-surface-border bg-surface-raised flex flex-col items-center justify-center gap-2 py-8'
              }`}
        ...
        <div className="flex gap-2 mt-5">
          ...
          <button className="btn btn-brand flex-1" onClick={submit} disabled={!name || !price}>
