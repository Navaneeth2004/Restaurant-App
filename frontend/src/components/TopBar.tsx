...
export default function TopBar({ view, setView }: Props) {
  ...
  return (
    <header className="h-14 flex-shrink-0 flex items-center px-4 gap-2 border-b border-surface-border bg-surface-card">
      ...
      {/* Right — fix: show name + role separately, not doubled */}
      <div className="ml-auto flex items-center gap-3">
        <div className="hidden sm:flex items-center gap-2">
          <div className="w-7 h-7 rounded-full bg-brand-500/20 border border-brand-500/30 flex items-center justify-center text-brand-400 text-xs font-bold font-display">
            {user?.name?.[0]?.toUpperCase()}
          </div>
          <div className="leading-none">
            <div className="text-xs font-medium text-white">{user?.name}</div>
            <div className="text-[10px] text-zinc-500 capitalize mt-0.5">{user?.role}</div>
          </div>
        </div>
        <button
          onClick={logout}
